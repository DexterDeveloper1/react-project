...
// using axios or local storage if base url is not available
...
export async function addItem(payload: Omit<IItem, "id">): Promise<IItem> {
    ...
    const next: IItem = {
        id: items.length ? Math.max(...items.map((i) => i.id)) + 1 : 1, ...payload
    };
    items.push(next);
    writeLocal(items);
    return next;
...
export async function updateItem(id: number, payload: Omit<IItem, "id">): Promise<IItem> {
    if (USE_API) {
